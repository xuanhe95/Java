...
public class PhoneBookWithHashMap {
    ...
    PhoneBookWithHashMap(){
        myMap = new HashMap<Integer,String>();
    } 
    
    public static void main(String[] args) {
        new PhoneBookWithHashMap().processQueries();
    }
    ...
    private void processQueryFast(Query query){
        ...
        if (query.type.equals("add")){
            myMap.put(query.number,query.name);
        } else if (query.type.equals("del")){
            myMap.remove(query.number);
        } else {
            String response = "not found";
            if(myMap.containsKey(query.number)){
                response = myMap.get(query.number);
            }
            writeResponse(response);
        } 
    }
