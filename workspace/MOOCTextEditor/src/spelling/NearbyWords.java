...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while ( !queue.isEmpty() && numSuggestions != 0 && times != 0) {
			String s = ((LinkedList<String>) queue).poll();
			List<String> nextWords = distanceOne(s, true);
			for (String nextWord :  nextWords) {
				if ( !visited.contains(nextWord) ) {
					visited.add(nextWord);
					queue.add(nextWord);
					if ( dict.isWord(nextWord)) {
						retList.add(nextWord);
						numSuggestions --;
					}
				} 
			}
   ...
   public static void main(String[] args) {
	   ...
	   word = "aim";
	   List<String> suggest2 = w.suggestions(word, 4);
	   System.out.println("Spelling Suggestions for \""+word+"\" are:");
