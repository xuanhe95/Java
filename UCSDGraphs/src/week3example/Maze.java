...
public class Maze {
	...
	public List<MazeNode> dfs(int startRow, int startCol, int endRow, int endCol) {
		...
		boolean found = search(start, goal, parentMap);
		
		if (!found) {
			System.out.println("No path exists");
			return new LinkedList<MazeNode>();
		}
		List<MazeNode> path = constructPath(start, goal,parentMap);
		return path;
	}
	
	private boolean search(MazeNode start, MazeNode goal, HashMap<MazeNode, MazeNode> parentMap) {
